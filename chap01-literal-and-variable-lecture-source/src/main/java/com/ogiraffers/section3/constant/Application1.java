...
public class Application1 {
    public static void main(String[] args)  {
         ...
         *   1-2. 예약어는 사용이 불가능하다.
         *         예약어? : 자바에서 미리 사용하겠다고 컴파일러와 약속한 키워드 등을 의미한다.
         *         외울 필요는 없지만 신경써서 변수명을 지을 것
         *   1-3. 변수명은 대문자와 소문자를 구분합니다.(대문자 소문자 다르면 다른 변수)
         *   1.4. 변수명은 숫자로 시작할 수 없다.
         *   1-5. 특수 기호는 '_'와 '$'만 사용 가능하다. (공백을 사용할 수 없습니다,)
         *
         * 2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 규칙(암묵적 규칙)
         *   2-1. 변수명의 길이 제한은 없다.(그러나 적당한 길이 권장. 한두 단어정도)
         *   2-2. 변수명이 합성어로 이루어진 경우, 첫 단어는 소문자, 두 번째 시작단어부터는 대문자로 시작한다.(camel-case)
         *   2-3. 단어와 단어 사이의 연결은 언더스코어(_)로 하지 않는다. (2-2 지켜라)
         *        상수를 사용할 때 특수하게 사용한다.
         *   2-4. 한글로 변수명을 짓는 게 가능하기는 하지만 권장되지 않음. (한글을 취급하는 다양한 방식 때문에 에러 유발)
         *   2.5. 변수 안에 저장된 값이 어떤 의미를 가지는지 명확하게 표현하도록 한다.
         *   2-6. 전형적인 변수 이름이 있다면 가급적 사용하도록 한다. (count sum avg 등등)
        ...
        // int a ge : // 사용 가능한 특수문자 외에는 사용이 불가능하다.

        int _age = 20; // 언더스코어(_)는 사용이 가능하다. 처음 중간 마지막 상관 없음.
        int Sharp$ = 20; // $ 사용 가능하다. 처음 중간 마지막 상관 없음.

        // 암묵적 약속
        // 변수의 길이에는 제한이 없지만 적당히 하자.
        int pneumonoultramicroscopicdsfdsfsf; // 이렇게는 사용하지 말자.

        /*
        * 카멜 케이스
        *
        * 합성어일 경우 첫 단어는 모두 소문자, 두 번째 시작 단어 이후로는 모두 대문자를 사용한다.
        * 자바에서는 클래스명만 유일하게 대문자로 시작한다.
        ...
        String s;    // 변수가 어떤 의미인지 파악하기 어려움
        String name; // 변수가 어떤 의미인지 파악하기 쉬움
