...
public class Car {
     ...
     *
     *  */
    private boolean isOn; // 기본 false false: 시동이 꺼짐
    ...
    public void startUp() {
        ...
        public void go() {
            if (isOn) {
                System.out.println("차가 앞으로 출발합니다.");
                this.speed += 10;
                System.out.println("현재 차의 시속은" + this.speed + "km/h 입니다.");
            } else {
                System.out.println("차에 시동이 걸려 있지 않습니다. 시동을 먼저 걸어주세요.");
            }
        }



    // 시동이 걸려 있고, 달리는 상태일 때만 멈출 수 있다.
    public void stop() {
        if (isOn) {
            ...
            if (this.speed > 0) {
                ...
                System.out.println("저는 이미 멈춰 있는 상태입니다.");
            }
        }  else{
                System.out.println("차에 시동이 걸려 있지 않습니다. 시동을 먼저 걸어 주세요.");
            }

        }

        // 자동차가 달리는 중이면 시동을 끌 수 없다. 시동이 꺼져 있으면 끌 수 없다.

    public void turnOff() {
        if(isOn) {
            if (speed > 0) {
                System.out.println("달리는 상태에서는 시동을 끌 수 없습니다. 차를 우선 멈춰 주세요.");
            } else {
                this.isOn = false;
