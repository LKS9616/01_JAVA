...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * 객체 지향 프로그래밍이란
        *
        * 현실세계의 모든 시간(event)는 객체와 객체의 상호작용에 의해 일어난다는 세게관을 프로그램을 만들 때 이용하여 새로운 세계를 창조하는 방법론이다.
        * 현실세계와 닮아있지만 현실세게와 다른 점은, 객체간의 상호 작용을 한다는 사실은 현실 세계와 동일하지만 의인화 기법이 적용되어 현실세계에서
        *  불가능한 무생물이나 개념 같은 존재들도 하나의 주체로 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 현실 세계와는 다른 점
        * 모든 객체는 책임을 가지고 자신의 역할을 수행해야 한다.
        * 또한 하나의 객체는 너무 많은 일을 수행하는 것이 아닌 적절한 책임을 가지고 있어야 한다. => 단일책임의 원칙
        * */

        /*
        * 객체를 설계하기 위해서는 현실세계를 그대로 반영하기에는 너무 방대하고 복잡하기 때문에 현실 세계를 프로그램의 목적에 맞게 단순화하는 추상화라는 기법을
        * 적용하게 된다.
        * */
        ...
        * 객체의 재사용성이 증가하면 중복되는 코드를 줄일 수 있고 오류 발생을 감소시킬 수 있다.*/
        ...
        /*
        * 객체와 객체의 상호작용
        * 객체와 객체는 메시지(메소드)를 통해 서로 상호작용을 한다. 이 때 메시지를 보내는 쪽은 송신자, 메시지를 수신하는 쪽을 수신자라고 한다.
        * 수신자는 메시지를 전달 받아 그 메시지에 해당하는 내용을 처리하는 방법을 스스로 결정하고,
        * 그 결정할 방법대로 처리할 명령어들을 순차적으로 기술한 것 ->메소드
        *
        * 상호작용에 중요한 것은 메시지를 송수신하고 그 방법을 기술하여 메시지에 응답하는 상호작용을 설계하는 것
        * -> 따라서 객체를 설계할 때는 속성은 부가적이며 적절한 책임을 설계하고 그 객체를 구현하는 방법으로 설계해 클래스를 작성하는 것.
        * */
        ...
        * 2. 자동차는 시동걸기, 앞으로 가기, 시동끄기를 할 수 있다
        * 3. 자동차는 처음에 멈춘 상태로 대기하고 있다.
        * 4, 카레이서는 먼저 자동차에 시동을 건다. (이미 걸려있는 경우 다시 시동을 걸 수 없다.)
        * 5. 카레이서가 엑셀레이터를 밟으면 시동이 걸린 상태인 경우, 자동차는 시속이 10km/h로 증가하여 앞으로 나간다.
        ...
        *
        * 2. 객체간 상호작용
        * 1) 카레이서가 수신할 수 있는 메시지(카레이서가 해야 하는 일) -> 자동차를 움직이게 해야 함.
        *   -시동을 걸어라
        *   -액셀레이터를 밟아라
        *   -브레이크를 밟아라
        *   -시동을 꺼라
        ...
        * 2) 가속 하기
        *   -사용자 -> 악셀레이터를 밟아라 -> 카레이서 -> 앞으로 가라-> 자동차
        * 3) 정지 하기
