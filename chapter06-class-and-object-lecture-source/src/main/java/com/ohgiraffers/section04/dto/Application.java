...
public class Application {
    public static void main(String[] args) {
        ...
        /*
        * 1. 생성자를 이용한 초기화
        * 장점 : setter 메서드를 여러 번 호출해서 사용하지 않고, 단 한 번의 호출로 인스턴스 생성 및 초기화 가능
        * 단점 : 필드를 초기화할 매개변수의 갯수를 경우의 수 별로 모두 만들어둬야 한다. 호출 시 인자가 많아지는 경우 어떠한 값이 어떤 필드를 의미하는지 한 눈에
        * 들어오지 않는다.
        * 예시) 아이디, 비밀번호, 이름이 모두 ohgiraffers 인 경우 new User("ohgiraffers", "ohgiraffers", "ohgiraffers);
        *
        * 2. 설정자를 이용한 초기화
