...
public class Application {
    ...
    /*
     * DTO 만드는 이유: 캡슐화의 원칙에는 일부 어긋나긴 하지만, 다른 목적을 가진 클래스와 객체를 추상화하는 기법
     * -> DTO(Data Transfer Object)
     * 이러한 객체를 설계할 때는 행위가 ㅏ아닌 데이터가 위주이며, 캡슐화의 원칙을 준수하여 모든 필드를 private으로 직접 접근을 막고
     * 각 필드값을 변경하거나 반환하는 메소드를 세트로 미리 작성해둔다.
     * private 필드와 필드값을 수정하는 설정자(setter) 필드에 접근하는 접근자(getter) 주로 계층간 데이터를 주고 받을 목적으로 사용한다.
     * */
