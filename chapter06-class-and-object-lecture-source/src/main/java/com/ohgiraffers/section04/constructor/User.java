...
public class User {
     ...
     * 생성자의 작성 위치
     *
     * 문법상으로 클래스 내부에 작성하면 되지만, 통상적으로 필드 선언부와 메소드 선언부 사이에 적는 것이 관례
     * */
     ...
     * 3. 작성한 생성자 외에는 인스턴스를 생성하는 방법을 제공하지 않겠다는 의미 -> 인스턴스를 생성하는 방법을 제한하기 위한 용도로 사용(초기값 전달 강제)
     * */
     ...
     * 2. 매개변수 있는 생성자
     *
     * 매개변수가 있는 생성자가 한 개라도 존재하면 기본생성자를 자바 컴파일러가 자동으로 추가하지 않는다.
     * 기본 생성자가 필요한 경우에는 반드시 명시적으로 작성을 해야 한다.
     *
     * */
    public User() {
    ...
    public String getInfo() {
        return "User [id=" + this.id + ", pwds" + this.pwd + ", " + "name=" + this.name + ", enrollDate=" + this.enrollDate + "]";
    }
