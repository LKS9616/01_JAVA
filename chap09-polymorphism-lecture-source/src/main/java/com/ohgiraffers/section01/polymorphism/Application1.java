...
public class Application1 {
    ...
    * 다형성
    *
    * 다형성이란 하나의 인스턴스가 여러 가지 타입을 가질 수 있는 것을 의미한다. 그렇기 때문에 하나의 타입으로 여러 타입의 인스턴스를 처리할 수 있기도 하고, 하나의 메소드 호출로 객체별로
    * 각기 다른 방법으로 동작하게 할 수도 있다.
    *
    * 다형성은 객체지향 프로그래밍의 4대 특징(캡슐화, 상속, 추상화, 다형성) 중 하나이며, 활용성이 높고, 장점이 많다. 하지만 학습이 어렵다.
    * */

    /*
    * 다형성 장점
    * 1. 여러 타입의 객체를 하나의 타입으로 관리할 수 있기에 유지보수성과 생산성 증가
    * 2. 상속을 기반으로 한 기술이기 때문에 상속관계에 있는 모든 객체는 동일한 메시지를 수신할 수 있다. 동일한 메시지를 수신하고 처리할 때 객체별로 다르게 할 수 있다는 장점을 가짐.
    * 3. 확장성이 좋은 코드를 작성할 수 있다.
    * 4. 결합도를 낯춰서 유지보수성을 증가시킬 수 있다.
    * */
...
//        Tiger t = new Animal();
        ...
        /*
        * 동적 바인딩
        * 컴파일 당시에는 해당 타입의 메소드와 연결이 되어있다가 런타임 당시 실제 객체가 가진 오버라이딩된 메소드로 바인디잉 바뀌어 동작하는 것
        * */

        a1.cry();
        a2.cry();

        // 현재 레퍼런스 변수의 타입이 Animal이기 때문에 Rabbit과 Tiger가 가지고 있는 고유한 기능은 동작시키지 못한다.
//        a1.jump();
//        a2.bite();
